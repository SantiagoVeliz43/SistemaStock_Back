...
export class PacienteService {
    ...
    try {
      ...
      if (existingPaciente) {
        throw new ConflictException(
          `El paciente con nombre "${data.nro_legajo}" ya existe.`,
        );
      }
  ...
  async findOne(nro_legajo: number) {
    ...
    if (!paciente) {
      throw new NotFoundException(
        `Paciente con legajo ${nro_legajo} no encontrado`,
      );
    }
  ...
  async update(nro_legajo: number, data: UpdatePacienteDto) {
    ...
    if (!exists) {
      throw new NotFoundException(
        `No se puede actualizar: paciente con legajo ${nro_legajo} no existe o está eliminado`,
      );
    }
  ...
  async remove(nro_legajo: number) {
    ...
    if (!exists) {
      throw new NotFoundException(
        `No se puede eliminar: paciente con legajo ${nro_legajo} no existe o ya está eliminado`,
      );
    }
